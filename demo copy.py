...
model = SimulationModel(**{
    ...
    'background_depth': cv2.resize(np.load("/home/yuezk/yzk/geltip-simulation/experimental_setup/geltip/sim_assets/bkg.npy"), sim_size),
    # 'cloud_map': cloud,
    ...
    template="""
        <mujoco>
            ...
            <asset>
                ...
                <mesh name="object" file="geltip_dataset/object_set/${object}.stl" scale="0.001 0.001 0.001"/>
            </asset>
    ...
    def save_frame(self, key):
        ...
        if self.config['dataset_name'].split('_')[0] == 'sim':
            with open(frame_path + '.npy', 'wb') as f:
                ...
                # sigma = 1  # 设置高斯滤波的标准差
                # depth_frame_filterded = gaussian_filter(depth_frame, sigma=sigma)

                tactile_rgb = cv2.cvtColor(model.generate(cv2.resize(depth_frame, sim_size)), cv2.COLOR_RGB2BGR)
                cv2.imshow('frame', depth_frame)
                cv2.imshow('tactile_rgb', tactile_rgb)
                cv2.waitKey(-1)
                cv2.imwrite('/home/yuezk/yzk/geltip-simulation/result/cylinder_shell_'+key+'.png', tactile_rgb)
